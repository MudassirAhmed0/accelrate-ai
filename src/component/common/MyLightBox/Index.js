...
const MyLightBox = ({
  ...
  const handleClose = () => {
    lightboxContainerRef.current.classList.add("opacity-0");
    lightboxContainerRef.current.classList.add("pointer-events-none");
    stopPlayingVideos();
    setShowNow(false);
  };
          ...
          {showNow &&
            sources.map((source, index) => {
              ...
              return (
                ...
                >
                  <span
                    className="w-full h-full top-0 left-0 absolute bg-black bg-opacity-[0.9]"
                    onClick={handleClose}
                  ></span>
                  {isVideo ? (
                    <video
